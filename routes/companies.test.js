...
describe("POST /companies", function () {
  ...
  test("not ok for non-admin users", async function () {
    ...
    //don't think we need to test that an error is thrown specifically,
    //since we test for that in the ensureAdmin middleware tests
  });
  ...
  test("non-admin-- unauthorized request w/ invalid data", async function () {
    const resp = await request(app)
      .post("/companies")
      .send({
        ...newCompany,
...
describe("PATCH /companies/:handle", function () {
  ...
  test("admin-- not found where no such company", async function () {
    const resp = await request(app)
  ...
  test("non-admin-- unauthorized for no such company", async function () {
    const resp = await request(app)
      .patch(`/companies/nope`)
      .send({
        name: "new nope",
  ...
  test("non-admin-- unauth bad request on handle change", async function () {
    const resp = await request(app)
      .patch(`/companies/c1`)
      .send({
        handle: "c1-new",
  ...
  test("non-admin--unauth bad request on invalid data", async function () {
    const resp = await request(app)
      .patch(`/companies/c1`)
      .send({
        logoUrl: "not-a-url",
...
describe("DELETE /companies/:handle", function () {
  ...
  test("non-admin--unauth not found for no such company", async function () {
    const resp = await request(app)
      .delete(`/companies/nope`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
